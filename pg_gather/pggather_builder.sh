...
get_sources(){
    ...
    mkdir debian rpm
    cd debian 
    wget https://raw.githubusercontent.com/surbhat1595/postgres-packaging/${BRANCH}/pg_gather/debian/rules
    wget https://raw.githubusercontent.com/surbhat1595/postgres-packaging/${BRANCH}/pg_gather/debian/control
    wget https://raw.githubusercontent.com/surbhat1595/postgres-packaging/${BRANCH}/pg_gather/debian/percona-pg-gather.install
    cd ../
    echo 9 > debian/compat
    cd rpm
    wget https://raw.githubusercontent.com/surbhat1595/postgres-packaging/DISTPG-630/pg_gather/rpm/pg_gather.spec
    cd ${WORKDIR}
    #
    ...
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${BRANCH}/${REVISION}/${BUILD_ID}" >> pg_gather.properties
...
get_tar(){
    TARBALL=$1
    TARFILE=$(basename $(find $WORKDIR/$TARBALL -name '*gather*.tar.gz' | sort | tail -n1))
    if [ -z $TARFILE ]
    then
        TARFILE=$(basename $(find $CURDIR/$TARBALL -name '*gather*.tar.gz' | sort | tail -n1))
        if [ -z $TARFILE ]
        then
