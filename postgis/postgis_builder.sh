...
get_sources(){
    ...
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${PSM_BRANCH}/${REVISION}/${BUILD_ID}" >> percona-postgis.properties
...
install_sfcgal(){
    wget https://gitlab.com/Oslandia/SFCGAL/-/archive/v1.3.10/SFCGAL-v1.3.10.tar.gz $CURDIR/SFCGAL-v1.3.10.tar.gz
    tar xvf $CURDIR/SFCGAL-v1.3.10.tar.gz
    cd $CURDIR/SFCGAL-v1.3.10
    cmake . && make && make install
    chmod 777 /usr/local/lib/libSFCGAL.*
    cp /usr/local/lib/libSFCGAL.* /usr/lib/postgresql/12/lib
...
build_srpm(){
    ...
    rpmbuild -bs --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .generic" \
        --define "version ${VERSION}" --define "pginstdir /usr/pgsql-12"  \
...
build_rpm(){
    ...
    rpmbuild --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .$OS_NAME" --define "version ${VERSION}" --define "pginstdir /usr/pgsql-12" --rebuild rpmbuild/SRPMS/$SRC_RPM
